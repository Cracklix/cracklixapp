        ...
        <main className="flex-1 p-8 md:p-12 overflow-y-auto no-scrollbar">
          <div className="max-w-7xl mx-auto space-y-12">
            <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
              <div className="space-y-2">
                 <div className="flex items-center gap-3">
                   ...
                   <h1 className="font-headline text-5xl font-black tracking-tighter uppercase leading-none">Pass Management</h1>
                 </div>
                 <p className="text-zinc-500 font-medium ml-1">Configure granular access tiers and SaaS monetization rules.</p>
              </div>
              <div className="p-6 px-10 rounded-[32px] bg-zinc-900 border border-white/5 flex items-center gap-12 shadow-2xl">
            ...
            <Tabs defaultValue="plans" className="space-y-10">
              <TabsList className="bg-zinc-900 border-white/5 p-1.5 rounded-[24px] h-16 w-fit shadow-lg">
              ...
              <TabsContent value="builder" className="animate-in fade-in slide-in-from-bottom-2">
                   ...
                   <div className="grid lg:grid-cols-12 gap-16 relative z-10">
                      ...
                      <div className="lg:col-span-4 space-y-10">
                         <div className="space-y-3">
                            <label className="text-[10px] font-black uppercase text-zinc-300 tracking-[0.3em] px-2">Pass Identity (Name)</label>
                            <Input 
                              placeholder="e.g. Punjab Police Elite PASS" 
                              value={newPlan.name}
                              onChange={e => setNewPlan({...newPlan, name: e.target.value})}
                              className="h-14 bg-black/40 border-white/10 rounded-2xl text-lg font-bold px-6 focus:border-primary/50"
                            />
                         </div>
                         <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-3">
                               <label className="text-[10px] font-black uppercase text-zinc-300 tracking-[0.3em] px-2">Price (INR)</label>
                               <Input 
                                 type="number" 
                                 value={newPlan.price}
                                 onChange={e => setNewPlan({...newPlan, price: Number(e.target.value)})}
                                 className="h-14 bg-black/40 border-white/10 rounded-2xl font-black text-xl text-emerald-500 px-6"
                               />
                            </div>
                            <div className="space-y-3">
                               <label className="text-[10px] font-black uppercase text-zinc-300 tracking-[0.3em] px-2">Duration (Days)</label>
                               <Input 
                                 type="number" 
                                 value={newPlan.duration}
                                 onChange={e => setNewPlan({...newPlan, duration: Number(e.target.value)})}
                                 className="h-14 bg-black/40 border-white/10 rounded-2xl font-black text-xl px-6"
                               />
                            </div>
                         </div>
                         <div className="space-y-3">
                            <label className="text-[10px] font-black uppercase text-zinc-300 tracking-[0.3em] px-2">Tier Rank</label>
                            <Select value={newPlan.tier} onValueChange={v => setNewPlan({...newPlan, tier: v as any})}>
                               <SelectTrigger className="h-14 bg-black/40 border-white/10 rounded-2xl font-bold px-6">
                                  <SelectValue />
                               </SelectTrigger>
                      ...
                      <div className="lg:col-span-8 grid md:grid-cols-2 gap-10">
                         <div className="space-y-8">
                            <div className="space-y-4">
                               ...
                               <div className="space-y-3 p-6 rounded-3xl bg-white/[0.02] border border-white/5 shadow-inner">
              ...
              <TabsContent value="audit" className="animate-in fade-in slide-in-from-bottom-2">
                 <div className="space-y-8">
                    <div className="flex justify-between items-center bg-zinc-900/50 p-10 rounded-[48px] border border-white/5 shadow-xl">
                       <div>
                       ...
                       <div className="relative w-96">
                          ...
                          <Input placeholder="Search identity signal..." className="h-12 bg-black/40 border-white/5 pl-12 rounded-xl text-sm font-bold" />
                       </div>
                    </div>
                             ...
                             {subscriptions.length > 0 ? subscriptions.map((sub) => (
                               <tr key={sub.id} className="hover:bg-white/[0.01] transition-colors group">
                                  ...
                                  <td className="px-12 py-10 text-right">
                                     <Button variant="ghost" size="icon" className="rounded-xl text-zinc-700 hover:text-destructive transition-colors"><Trash2 size={20} /></Button>
                                  </td>
                               </tr>
                             )) : (
